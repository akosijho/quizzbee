...
class NextViewModel extends AppViewModel {
  Question currentChallenge;
  Question? challenge;

  NextViewModel({required this.currentChallenge});
    ...
    if(event.eventName == 'my-event'){
      ...
      if (event.data.toString() != "\"[]\"") {
        ...
        if (challenge != currentChallenge) {
          // timer!.cancel();
          await nav.pushReplacementNamed(Routes.next_one,
              arguments: NextOneArguments(q: challenge!));
          notifyListeners();
        }
      }else{
